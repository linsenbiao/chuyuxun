...
@Component
public class ActivityThreadService implements BeanFactoryAware {

	private static Logger logger = LoggerFactory.getLogger(ActivityThreadService.class);
	...
	// 消息缓冲队列
	Queue<ActivityMessage> msgQueue = new LinkedList<ActivityMessage>();
	...
	final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
		...
		public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
			...
			msgQueue.offer(((ActivityDataBaseThread) r).getMsg());// 放入自定义的缓冲队列
		}
	};
		...
		public void run() {
			if (!msgQueue.isEmpty()) { // 先判断缓冲队列是否存在缓冲消息
				if (threadPool.getQueue().size() < WORK_QUEUE_SIZE) { // 判断线程池中的队列是否有空余
					logger.info("调度：");
					ActivityMessage message = (ActivityMessage) msgQueue.poll();// 取出缓冲队列中的消息进行处理
					ActivityDataBaseThread accessDBThread = (ActivityDataBaseThread) factory.getBean("dBThread");
	...
	public Queue<ActivityMessage> getMsgQueue() {
		return msgQueue;
	}

	// 将任务加入订单线程池
	public void processOrders(ActivityMessage message) {
